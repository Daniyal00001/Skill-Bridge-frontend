...
const InvitationsPage = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status?.toUpperCase()) {
      case "ACCEPTED":
        return <Badge className="bg-emerald-500/15 text-emerald-700 hover:bg-emerald-500/25 border-emerald-200">Accepted</Badge>;
      case "REJECTED":
        return <Badge variant="destructive" className="bg-red-500/15 text-red-700 hover:bg-red-500/25 border-red-200">Rejected</Badge>;
      case "CANCELLED":
        return <Badge variant="secondary" className="bg-zinc-500/15 text-zinc-700 hover:bg-zinc-500/25 border-zinc-200">Cancelled</Badge>;
      case "PENDING":
    ...
    <DashboardLayout>
      <div className="container mx-auto p-4 md:p-8 space-y-8 animate-fade-in">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Project Invitations</h1>
            ...
            <CardContent className="p-0">
                    ...
                    <TableRow key={invite.id} className="group hover:bg-muted/50 transition-colors">
                      <TableCell className="pl-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-semibold text-sm">{invite.clientName}</p>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell className="py-4 font-medium text-sm line-clamp-1 max-w-[200px]" title={invite.projectTitle}>
                        {invite.projectTitle}
                      </TableCell>
              ...
              {filteredInvites.length === 0 && (
                <div className="text-center py-16 px-4">
                  ...
                  <h3 className="text-lg font-bold mb-1">No Invitations Found</h3>
        ...
        <Dialog open={detailsModalOpen} onOpenChange={setDetailsModalOpen}>
          <DialogContent className="max-w-4xl border-none bg-card/95 backdrop-blur-2xl rounded-[2.5rem] shadow-2xl overflow-hidden max-h-[92vh] flex flex-col p-0 border border-white/10">
            <DialogHeader className="p-8 pb-6 border-b shrink-0 bg-secondary/30 relative overflow-hidden">
              ...
              <DialogTitle className="text-3xl font-black tracking-tight mb-1">Project Terms Details</DialogTitle>
              <DialogDescription className="text-sm font-medium text-muted-foreground/80">
                Formal proposal from <span className="text-foreground font-bold">{selectedInvite.clientName}</span> for the project "{selectedInvite.projectTitle}".
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="p-8 overflow-y-auto space-y-8 flex-1 custom-scrollbar">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-primary/[0.03] border border-primary/10 rounded-3xl p-6 space-y-4">
                      <div className="flex items-center justify-between">
                        <h4 className="text-[11px] font-black uppercase tracking-[0.2em] text-primary flex items-center gap-2">
                          <Paperclip className="w-4 h-4" /> Attached Project Files
                        </h4>
                  ...
                  {selectedInvite.message ? (
                    <div className="bg-card border rounded-3xl p-6 shadow-sm border-muted/40 italic text-muted-foreground relative">
                      ...
                      <div className="mt-4 pt-4 border-t flex items-center gap-2">
                        <Avatar className="w-6 h-6">
                            ...
                            <AvatarFallback>{selectedInvite.clientName.charAt(0)}</AvatarFallback>
                        </Avatar>
                        <span className="text-[10px] uppercase font-black tracking-widest text-muted-foreground">{selectedInvite.clientName}</span>
                      </div>
                    </div>
                  ) : (
                    <div className="p-6 bg-muted/10 border-2 border-dashed rounded-3xl flex flex-col items-center justify-center gap-2 opacity-50 text-center py-10">
                      ...
                      <p className="text-xs font-bold text-muted-foreground tracking-tight">No message provided with invitation</p>
                    </div>
                  )}
                </div>
                  ...
                  <div className="p-6 rounded-3xl border bg-gradient-to-br from-primary/10 to-transparent space-y-4 shadow-sm">
                    ...
                    <div className="pt-4 border-t border-primary/10 grid grid-cols-1 gap-4">
                        {(!selectedInvite.milestones || selectedInvite.milestones.length === 0) && (
                          <div className="space-y-1">
                              <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">General Revisions</p>
                              <p className="text-lg font-bold flex items-center gap-2"><RotateCcw className="w-4 h-4 text-primary" /> {selectedInvite.revisionsAllowed} Available</p>
                          </div>
                        )}
                        <div className="space-y-1">
                            <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Proposal Date</p>
                  ...
                  {selectedInvite.milestones && selectedInvite.milestones.length > 0 ? (
                    ...
                    <div className="p-12 text-center border-2 border-dashed rounded-[2rem] bg-secondary/10 space-y-3">
                         ...
                         <div className="space-y-1">
                            ...
                            <p className="text-sm text-muted-foreground font-medium max-w-sm mx-auto">This invitation is for a single-delivery project without structured milestones.</p>
                         </div>
                    </div>
                  )}
                </div>
              ...
              <div className="bg-amber-500/10 border border-amber-500/20 p-6 rounded-[1.5rem] text-sm font-semibold text-amber-800 space-y-2">
                <div className="flex items-center gap-2">
                    <Badge className="bg-amber-500 text-white font-black uppercase px-2 py-0.5 text-[9px] border-none">Important</Badge>
                </div>
