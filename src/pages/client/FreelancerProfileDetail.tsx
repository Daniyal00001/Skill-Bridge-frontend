...
const FreelancerProfileDetail = () => {
    ...
    return (
      <DashboardLayout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
          ...
          <p className="text-slate-400 font-bold uppercase tracking-widest text-[11px]" style={{ fontFamily: "'Sora', sans-serif" }}>
            Loading Profile…
          </p>
        </div>
      </DashboardLayout>
  ...
  const memberSince = freelancer.user?.createdAt ? new Date(freelancer.user.createdAt).getFullYear() : "—";
  const lastActive = freelancer.user?.lastActiveAt ? (() => {
    const diff = Date.now() - new Date(freelancer.user.lastActiveAt).getTime();
  ...
  const languages = Array.isArray(freelancer.languages)
    ? freelancer.languages.map((l: any) => {
        ...
        if (typeof l === "object" && l !== null) {
          return { lang: l.lang || l.name || l.label || "Unknown", level: l.level || null };
        }
  ...
  return (
    <DashboardLayout>
      ...
      <div className="fp-root max-w-6xl mx-auto py-6 px-4 pb-28 md:pb-10">
        <div className="mb-6">
          <Link to="/client/browse" className="inline-flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.18em] text-slate-400 hover:text-slate-700 transition-colors">
            ...
            <div className="flex flex-col md:flex-row gap-5 md:items-end justify-between min-h-[110px]">
              <div className="flex flex-col md:flex-row gap-5 md:items-end">
                <div className="relative shrink-0">
                  <Avatar className="h-28 w-28 ring-4 ring-white shadow-lg border border-slate-100">
                    <AvatarImage src={freelancer.user?.profileImage} alt={freelancer.fullName} />
                    <AvatarFallback className="text-2xl font-black bg-gradient-to-br from-slate-100 to-slate-200 text-slate-500">
                      {initials}
                    </AvatarFallback>
                  </Avatar>
                  {freelancer.availability === "AVAILABLE" && <span className="absolute bottom-1.5 right-1.5 w-4 h-4 bg-emerald-400 rounded-full ring-2 ring-white" />}
                </div>

                <div className="pb-1 space-y-2">
                  <div className="flex flex-wrap items-center gap-2">
                    <h1 className="text-2xl md:text-[28px] font-black text-slate-900 tracking-tight leading-none">{freelancer.fullName}</h1>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <div className="flex items-center gap-1">
                      {[1, 2, 3, 4, 5].map(s => (
                        <Star key={s} className={cn("w-4 h-4", s <= Math.round(freelancer.averageRating || 5) ? "fill-amber-400 text-amber-400" : "fill-slate-100 text-slate-200")} />
                      ))}
                      <span className="fp-mono text-sm font-medium text-slate-800 ml-1">{(freelancer.averageRating || 5).toFixed(1)}</span>
                      <span className="text-slate-400 text-xs font-medium ml-0.5">({freelancer.totalReviews || 0} reviews)</span>
                    </div>
              ...
              <div className="flex flex-col gap-3 md:pb-1 shrink-0">
                <div className="flex items-center gap-2">
                  <Button variant="outline" className="h-10 rounded-xl font-bold border-slate-200 text-slate-600 hover:bg-slate-50 text-xs px-4" onClick={handleMessage} disabled={isMessaging}>
                    {isMessaging ? <Loader2 className="w-4 h-4 mr-1.5 animate-spin" /> : <MessageSquare className="w-4 h-4 mr-1.5" />}
                    {isMessaging ? "Connecting…" : "Message"}
                  </Button>
                  <Button className="h-10 rounded-xl font-bold px-5 text-xs bg-slate-900 hover:bg-slate-800 shadow-sm" onClick={() => setIsInviteModalOpen(true)}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          <StatPill icon={Star} label="Avg Rating" value={(freelancer.averageRating || 5).toFixed(1)} accent="bg-amber-400" />
          <StatPill icon={Briefcase} label="Projects" value={freelancer.totalReviews || 0} accent="bg-emerald-500" />
          <StatPill icon={TrendingUp} label="Hourly Rate" value={freelancer.hourlyRate ? `$${freelancer.hourlyRate}` : "—"} accent="bg-sky-500" />
          <StatPill icon={BarChart3} label="Profile Score" value={`${freelancer.profileCompletion || 0}%`} accent="bg-violet-500" />
        </div>

        <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-2xl mb-6 overflow-x-auto">
          {tabs.map(tab => (
            <button key={tab} className={cn("fp-tab", activeTab === tab && "active")} onClick={() => setActiveTab(tab)}>{tab}</button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            {activeTab === "overview" && (
              <>
                <div className="fp-card p-6 overflow-hidden">
                  ...
                  <p className="text-slate-600 leading-relaxed font-medium text-sm break-words whitespace-pre-wrap">{freelancer.bio || "No bio provided yet."}</p>
                </div>
                    ...
                    <div className="space-y-3.5">
                      {freelancer.skills.map(s => (
                        <div key={s.id} className="flex items-center gap-4">
                          <span className="text-sm font-semibold text-slate-700 w-32 shrink-0">{s.skill.name}</span>
                          <div className="flex items-center gap-3 flex-1">
                            ...
                            <span className="fp-mono text-[10px] text-slate-400 font-medium whitespace-nowrap">
                              {["", "Beginner", "Basic", "Intermediate", "Advanced", "Expert"][s.proficiencyLevel || 3]}
                            </span>
                          </div>
                        </div>
                      ))}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {languages.map((l, i) => (
                        <div key={i} className="flex items-center gap-2 bg-slate-50 border border-slate-100 rounded-xl px-3 py-2">
                          <span className="text-sm font-semibold text-slate-700">{l.lang}</span>
                          {l.level && <span className="fp-mono text-[10px] text-slate-400">{l.level}</span>}
                        </div>
                      ))}
                    </div>
                  </div>
                        ...
                        <div key={item.id} className="fp-portfolio-card">
                          <div className="aspect-video bg-slate-100 relative overflow-hidden">
                            {item.imageUrl ? (
                              ...
                              <div className="w-full h-full flex items-center justify-center"><Code2 className="w-10 h-10 text-slate-200" /></div>
                            )}
                          </div>
                          <div className="p-4">
            ...
            {activeTab === "gigs" && (
              <div className="fp-card p-6">
                <SectionLabel>PACKAGED SERVICES (GIGS) ({freelancer.gigs?.length || 0})</SectionLabel>
                {freelancer.gigs?.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {freelancer.gigs.map(gig => (
                      <Link key={gig.id} to={`/client/gigs/${gig.id}`} target="_blank" className="group relative aspect-square rounded-[2.5rem] overflow-hidden bg-slate-100 shadow-lg hover:shadow-2xl transition-all duration-500 cursor-pointer">
                        <div className="absolute inset-0">
                          {gig.fileUrl ? (
                            <img src={gig.fileUrl} alt={gig.title} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" />
                          ) : (
                        ...
                        <div className="absolute top-6 right-6 scale-90 origin-right transition-transform group-hover:scale-100">
                           <div className="bg-white/10 backdrop-blur-md border border-white/20 px-3 py-1 rounded-full text-[10px] font-black text-white uppercase tracking-tighter">PREMIUM SERVICE</div>
                        </div>
                        <div className="absolute inset-0 p-8 flex flex-col justify-end text-left font-bold">
                          <h4 className="text-2xl font-black text-white tracking-tighter uppercase leading-none mb-2 transform transition-transform group-hover:-translate-y-1">{gig.title}</h4>
                          <div className="flex items-center justify-between pt-4 border-t border-white/10 mt-2 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-4 group-hover:translate-y-0">
                            <span className="flex items-center gap-2 text-[10px] font-bold text-white/60 uppercase tracking-[0.2em]">
                              <Sparkles className="w-3 h-3 text-emerald-400" /> Exclusive Offer
                            </span>
                            <Button className="h-9 px-5 bg-white text-slate-900 hover:bg-emerald-400 hover:text-white rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all">View Details</Button>
                          </div>
                        </div>
                      </Link>
                    ))}
                  ...
                  <div className="py-20 text-center">
                    ...
                    <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">No services listed yet</p>
                  </div>
                )}
              </div>
            )}
              ...
              <div className="fp-card p-6">
                <div className="flex items-center gap-6 mb-6 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  <div className="text-center shrink-0">
                    <div className="text-4xl font-black text-slate-900 tracking-tight fp-mono">{(freelancer.averageRating || 5).toFixed(1)}</div>
                    <div className="flex items-center gap-0.5 justify-center mt-1">
                      {[1, 2, 3, 4, 5].map(s => <Star key={s} className={cn("w-3.5 h-3.5", s <= Math.round(freelancer.averageRating || 5) ? "fill-amber-400 text-amber-400" : "fill-slate-200 text-slate-200")} />)}
                    </div>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">{freelancer.totalReviews || 0} reviews</p>
                  </div>
                  <div className="flex-1 space-y-1.5">
                    {[5, 4, 3, 2, 1].map(star => {
                      const count = freelancer.reviews?.filter(r => Math.round(r.rating) === star).length || 0;
                      const pct = freelancer.totalReviews ? (count / freelancer.totalReviews) * 100 : 0;
                      return (
                        <div key={star} className="flex items-center gap-2">
                          <span className="fp-mono text-[10px] text-slate-400 w-4 text-right">{star}</span>
                          <div className="flex-1 h-1.5 bg-slate-200 rounded-full overflow-hidden">
                            <div className="h-full bg-amber-400 rounded-full" style={{ width: `${pct}%` }} />
                          </div>
                          <span className="fp-mono text-[10px] text-slate-400 w-4">{count}</span>
                        </div>
                      );
                    })}
                  </div>
                ...
                {freelancer.reviews?.length > 0 ? (
                        ...
                        <div className="flex items-start justify-between gap-4 mb-2">
                          <div className="flex items-center gap-3">
                            <Avatar className="h-8 w-8">
                              ...
                              <AvatarFallback className="text-[10px] font-black bg-slate-100 text-slate-500">{review.giver?.name?.charAt(0) || "C"}</AvatarFallback>
                            </Avatar>
                            <div>
                              <p className="text-sm font-bold text-slate-800">{review.giver?.name || "Anonymous"}</p>
                              <p className="text-[10px] text-slate-400 font-medium">{new Date(review.revealedAt || review.submittedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-0.5 shrink-0">
                            {[1, 2, 3, 4, 5].map(s => <Star key={s} className={cn("w-3.5 h-3.5", s <= review.rating ? "fill-amber-400 text-amber-400" : "fill-slate-100 text-slate-200")} />)}
                          </div>
                        </div>
                  ...
                  <div className="py-20 text-center text-slate-400 text-[10px] font-black uppercase tracking-widest">No public feedback yet</div>
                )}
              </div>
            )}
                      ...
                      {freelancer.educations.map(edu => (
                        <div key={edu.id} className="flex gap-4">
                          ...
                          <div>
                            ...
                            {edu.year && <p className="fp-mono text-[10px] text-slate-300 mt-0.5">{edu.year}</p>}
                          </div>
                        </div>
                      ))}
                    </div>
                  ) : <p className="text-slate-400 text-xs italic">No education history.</p>}
                </div>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between gap-2">
                              <div>
                                ...
                                <p className="text-xs text-slate-500 font-medium truncate">{cert.issuingOrganization}</p>
                              </div>
                              {cert.credentialUrl && (
                                <a href={cert.credentialUrl} target="_blank" rel="noreferrer" className="shrink-0 text-slate-300 hover:text-sky-500 transition-colors">
          ...
          <aside className="space-y-5">
            <div className="fp-card p-6">
              <div className="text-center mb-5 pb-5 border-b border-slate-100">
                <div className="fp-mono text-3xl font-black text-slate-900">{freelancer.hourlyRate ? `$${freelancer.hourlyRate}` : "TBD"}<span className="text-sm font-medium text-slate-400">/hr</span></div>
                <p className="text-[11px] font-bold uppercase tracking-widest text-slate-400 mt-1">Hourly Rate</p>
              </div>
              <div className="space-y-2">
                <Button className="w-full h-11 rounded-xl font-bold bg-slate-900 hover:bg-slate-800" onClick={() => setIsInviteModalOpen(true)}>Invite to Project</Button>
                <Button variant="outline" className="w-full h-11 rounded-xl font-bold border-slate-200" onClick={handleMessage} disabled={isMessaging}>Message</Button>
              </div>
            </div>
            ...
            <div className="fp-card p-5">
              ...
              <div className="space-y-3">
                {[
                  { label: "Email Verified", ok: freelancer.user?.isEmailVerified },
                  { label: "ID Verified", ok: freelancer.user?.isIdVerified },
                  { label: "Payment Verified", ok: freelancer.user?.isPaymentVerified },
                ].map(({ label, ok }) => (
                  <div key={label} className="flex items-center justify-between">
                    <span className="text-xs font-semibold text-slate-600">{label}</span>
                    <div className={cn("flex items-center gap-1 text-[10px] font-bold rounded-full px-2 py-0.5", ok ? "text-emerald-600 bg-emerald-50 border-emerald-100 border" : "text-slate-400 bg-slate-50 border-slate-100 border")}>
                      <CheckCircle2 className="w-3 h-3" /> {ok ? "Verified" : "Pending"}
                    </div>
                  </div>
                ))}
              </div>
      ...
      <InviteFreelancerModal isOpen={isInviteModalOpen} onClose={() => setIsInviteModalOpen(false)} freelancerId={id || ""} freelancerName={freelancer.fullName} />
    </DashboardLayout>
  );
};
