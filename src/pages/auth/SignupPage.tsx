...
import { Eye, EyeOff, ArrowLeft, Loader2, Briefcase, Code } from 'lucide-react'
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // calls real backend API via AuthContext → auth.service → api.ts
      ...
      <div className="hidden lg:flex lg:flex-1 gradient-hero items-center justify-center p-12">
        <div className="max-w-lg text-white">
          ...
          <p className="text-white/80 text-lg mb-8">
            {role === 'client'
              ? 'Post your project and get matched with pre-vetted freelancers.'
              : 'Get matched with projects that fit your skills.'}
          </p>
          <div className="space-y-4">
            {(role === 'client'
              ? ['AI-powered project scoping', 'Pre-vetted freelancers only', 'Secure milestone payments']
              : ['Smart project matching', 'Guaranteed secure payments', 'Build your portfolio']
            ).map((item) => (
