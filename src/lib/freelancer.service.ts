...
export const freelancerService = {
  ...
  updateOnboardingStep2: async (data: { hourlyRate: number, bio: string, availability: string, experienceLevel: string }) => {
  ...
  updateOnboardingStep5: async (data: { github?: string, linkedin?: string, portfolio?: string, website?: string }) => {
    const response = await api.post('/freelancers/onboarding/step-5', data)
    return response.data
  },

  uploadOnboardingFiles: async (formData: FormData) => {
    const response = await api.post('/freelancers/onboarding/upload', formData, {
      headers: {
