...
import { Paperclip, Send, ChevronUp, CheckCheck, Check, Loader2, FileText } from 'lucide-react'
...
function shouldShowDateSeparator(curr: ChatMessage, prev?: ChatMessage): boolean {
  if (!prev) return true
    ...
    >
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b bg-background/80 backdrop-blur-sm flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-sm tracking-tight">{other?.name ?? 'Unknown'}</h3>
          ...
          <div className="space-y-4 animate-pulse py-4">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className={cn('flex gap-2', i % 2 === 0 ? 'flex-row-reverse' : '')}>
                ...
                <div className={cn('h-10 rounded-2xl bg-muted', i % 2 === 0 ? 'w-40' : 'w-52')} />
              </div>
            ))}
          </div>
        )}
                  ...
                  <div className={cn('max-w-[72%] flex flex-col', isMe ? 'items-end' : 'items-start')}>
                    ...
                    <div
                      className={cn(
                        ...
                        isGrouped && isMe ? 'rounded-tr-2xl rounded-br-sm' : '',
                        isGrouped && !isMe ? 'rounded-tl-2xl rounded-bl-sm' : ''
                      )}
                    >
                      {msg.type === 'FILE' ? (
                        <AttachmentPreview
      ...
      <div className="px-4 py-3 border-t bg-background/80 backdrop-blur-sm flex-shrink-0">
        <div className="flex items-end gap-2">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {currentUserRole === 'CLIENT' && room.otherUser?.role === 'FREELANCER' && (
              <Button
                variant="ghost"
                size="icon"
      ...
      {currentUserRole === 'CLIENT' && room.otherUser?.role === 'FREELANCER' && (
        <SendContractModal
          ...
          freelancerId={room.otherUser.id}
