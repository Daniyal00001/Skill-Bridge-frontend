...
export const FreelancerCard = ({ freelancer: f, view = "grid" }: Props) => {
  ...
  const lastActiveDate = f.user?.lastActiveAt ? new Date(f.user.lastActiveAt) : null;
        ...
        <>
          <div className="flex flex-col md:flex-row md:items-center gap-5">
            ...
            <div className="relative">
              ...
              {isOnline && (
                <span className="absolute -bottom-1 -right-1 w-4 h-4 bg-emerald-500 rounded-lg ring-2 ring-white animate-pulse" />
              )}
            </div>
            ...
            <div className="flex-1 space-y-2">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                >
                  ...
                  {avail.label}
                  {isOnline && (
                    <span className="ml-1 text-[8px] font-black bg-emerald-100 text-emerald-600 px-1 py-0.5 rounded uppercase">Online</span>
                  )}
                </span>
        ...
        <div className="p-5 pb-3">
          <div className="flex items-start justify-between mb-3">
            ...
            <div className="relative shrink-0">
              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-400 to-violet-500 flex items-center justify-center text-white font-bold text-base overflow-hidden">
                {f.user?.profileImage ? (
                  <img
                    src={f.user.profileImage}
                    alt={f.fullName}
              ...
              {isOnline && (
                <span className="absolute -bottom-1 -right-1 w-3.5 h-3.5 bg-emerald-500 rounded-lg ring-2 ring-white animate-pulse" />
              )}
            </div>
          ...
          <div className="flex items-center gap-2 mt-1">
            <span className="flex items-center gap-1 text-[9px] text-slate-400 font-medium uppercase tracking-tighter">
              ...
              <span className={cn(isOnline ? "text-emerald-600 font-bold" : "")}>
                {lastActiveText}
              </span>
            </span>
          </div>
